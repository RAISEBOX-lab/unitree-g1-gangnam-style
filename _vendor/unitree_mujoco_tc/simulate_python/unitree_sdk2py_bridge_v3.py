...
class UnitreeSdk2Bridge:
    ...
    def PublishHighState(self):
        ...
        # --- NEW: publish IMU into SportModeState (same values as LowState.imu_state) ---
        if self.mj_data is not None and self.have_frame_sensor_:
            
            base = self.dim_motor_sensor
            # quaternion (w x y z) at indices base + 0..3
            self.high_state.imu_state.quaternion[0] = self.mj_data.sensordata[base + 0]
            self.high_state.imu_state.quaternion[1] = self.mj_data.sensordata[base + 1]
            self.high_state.imu_state.quaternion[2] = self.mj_data.sensordata[base + 2]
            self.high_state.imu_state.quaternion[3] = self.mj_data.sensordata[base + 3]
            # gyro (xyz) at indices base + 4..6
            self.high_state.imu_state.gyroscope[0]  = self.mj_data.sensordata[base + 4]
            self.high_state.imu_state.gyroscope[1]  = self.mj_data.sensordata[base + 5]
            self.high_state.imu_state.gyroscope[2]  = self.mj_data.sensordata[base + 6]
            # accel (xyz) at indices base + 7..9
            self.high_state.imu_state.accelerometer[0] = self.mj_data.sensordata[base + 7]
            self.high_state.imu_state.accelerometer[1] = self.mj_data.sensordata[base + 8]
            self.high_state.imu_state.accelerometer[2] = self.mj_data.sensordata[base + 9]

            print("IMU SENSOR DATA="+str(self.high_state.imu_state))
