...
GL_SHADOW           = getattr(config, "GL_SHADOW", False)      # OpenGL Effects → Shadow
GL_REFLECTION       = getattr(config, "GL_REFLECTION", False)  # OpenGL Effects → Reflection
GL_SKYBOX           = getattr(config, "GL_SKYBOX", False)      # OpenGL Effects → Skybox

locker = threading.Lock()


def _enum_member(enum_obj, candidates):
    """Return first existing enum member from list of candidate names."""
    for name in candidates:
        if hasattr(enum_obj, name):
            return getattr(enum_obj, name)
    # Debug aid: show what's available so you can adjust names if needed
    print("[GL] mjtRndFlag available:", [n for n in dir(enum_obj) if n.isupper() or n.startswith("mjRND_")])
    raise AttributeError(f"None of {candidates} exist in {enum_obj!r}")

def _apply_gl_flags(viewer, shadow: bool, reflection: bool, skybox: bool):
    rf = mujoco.mjtRndFlag
    # Try MuJoCo 2.x names first, then 3.x uppercase names
    SHADOW     = _enum_member(rf, ["mjRND_SHADOW",     "SHADOW"])
    REFLECTION = _enum_member(rf, ["mjRND_REFLECTION", "REFLECTION"])
    SKYBOX     = _enum_member(rf, ["mjRND_SKYBOX",     "SKYBOX"])

    scene = getattr(viewer, "_scene", None)
    if scene is None:
        return
    scene.flags[int(SHADOW)]     = bool(shadow)
    scene.flags[int(REFLECTION)] = bool(reflection)
    scene.flags[int(SKYBOX)]     = bool(skybox)
...
# Apply OpenGL Effects defaults from config (Shadow / Reflection / Skybox)
try:
    scene = getattr(viewer, "_scene", None)
    if scene is not None:
        scene.flags[mujoco.mjtRndFlag.mjRND_SHADOW]     = bool(GL_SHADOW)
        scene.flags[mujoco.mjtRndFlag.mjRND_REFLECTION] = bool(GL_REFLECTION)
        scene.flags[mujoco.mjtRndFlag.mjRND_SKYBOX]     = bool(GL_SKYBOX)
        viewer.sync()
except Exception as e:
    print(f"[WARN] Failed to set GL flags: {e}")
...
def PhysicsViewerThread():
    ...
    try:
        while viewer.is_running():
            locker.acquire()
            # Re-assert GL flags each frame; some viewer paths overwrite them.
            _apply_gl_flags(viewer, GL_SHADOW, GL_REFLECTION, GL_SKYBOX)
            viewer.sync()
            locker.release()
